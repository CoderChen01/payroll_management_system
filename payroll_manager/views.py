...
def employee_dashboard(request, emp_id):
    ...
    if user == request.user:
        ...
        return render(request, 'payroll_manager/employee_dashboard.html',
                      context={'user_info': user_info, 'user_paygrade': user_paygrade, 'user_pay': user_pay,
                               'user_achieve': user_achieve, 'user_leave': user_leave})
    else:
...
def admin_dashboard(request):
    if Account.objects.filter(user_id=request.user.user_id, is_employer=True):
        ...
        return render(request, 'payroll_manager/admin_dashboard.html',
...
def admin_employee_dashboard(request, emp_id):
    ...
    if TLeave.objects.filter(employee=user_info).exists():
        user_leave = TLeave.objects.filter(employee=user_info)
    return render(request, 'payroll_manager/admin_employee_dashboard.html',
                  context={'user_info': user_info, 'user_paygrade': user_paygrade, 'user_pay': user_pay,
                           'user_achieve': user_achieve, 'user_leave': user_leave})
...
def changePay(request, emp_id):
    ...
    if True:
        ...
        return render(request, 'payroll_manager/payChange.html',
                      context={'formPay': AddFormpay, 'formPaygrade': AddFormpaygrade})
